...
<head>
    ...
    <style>
        ...
        .cashier-header .kasir-title{font-size:18px;font-weight:600;color:#1e3a8a;flex:0 0 auto;white-space:nowrap;flex-shrink:0;margin-left:0}
        ...
        .cashier-sidebar{width:200px;background:var(--surface);border-radius:6px;padding:0;box-shadow:0 1px 4px rgba(0,0,0,0.03);height:fit-content}
        .cashier-sidebar .tab{padding:16px 18px;font-weight:500;cursor:pointer;border:none;outline:none;text-align:left;width:100%;background:#fff;color:#222;border-bottom:1px solid #e5e7eb;transition:background 0.2s}
        .cashier-sidebar .tab.active{background:var(--action);color:#fff;border-radius:6px 6px 0 0;}
        .cashier-sidebar .tab:last-child{border-radius:0 0 6px 6px;border-bottom:none}
        .cashier-main{flex:1;}
        .filter-box{background:var(--surface);padding:18px 18px 8px 18px;border-radius:8px;display:flex;flex-direction:column;gap:12px;margin-bottom:24px;border:1px solid rgba(0,0,0,0.06);box-shadow:0 1px 4px rgba(0,0,0,0.03)}
        .filter-row{display:flex;gap:12px;align-items:center;flex-wrap:wrap}
        .filter-row input[type="text"]{flex:1;padding:10px 14px;border-radius:8px;border:1px solid #e5e7eb}
        .filter-row label{font-size:13px;color:#64748b;margin-bottom:2px;display:block}
        .filter-row input[type="date"]{padding:8px;border-radius:6px;border:1px solid #e5e7eb}
        .filter-row button{background:#64748b;color:#fff;padding:10px 18px;border-radius:6px;border:none;box-shadow:1px 2px 6px #e5e7eb}
        .action-btns{display:flex;gap:8px;margin-bottom:18px;flex-wrap:wrap}
        .action-btns button{padding:10px 18px;border-radius:6px;border:none;cursor:pointer;font-weight:500;box-shadow:1px 2px 6px #e5e7eb}
        .action-btns .add{background:var(--action);color:#fff}
        .action-btns .export{background:var(--surface);color:var(--action);border:1px solid var(--action)}
                ...
                <div class="header-actions">
                    <div class="header-hd">
                        <div class="header-logo">
                            <img src="https://ui-avatars.com/api/?name=HDS&background=e6eef6&color=2563eb&size=64&rounded=true" alt="logo">
                        </div>
                        <button class="header-dropdown-btn" id="hdDropdownBtn">hangleki... <i class="fas fa-chevron-down"></i></button>
        ...
        <div class="cashier-content">
            ...
            <div class="cashier-main">
                <div id="pembayaran-section">
                    <div class="filter-box">
                        <form class="filter-row" onsubmit="event.preventDefault(); alert('Filter diklik!');">
                            ...
                            <button type="submit" id="filterBtn"><i class="fas fa-search"></i></button>
                        </form>
                ...
                <div id="hutang-section" style="display:none;">
                    <div class="filter-box">
                        <form class="filter-row" onsubmit="event.preventDefault(); alert('Filter diklik!');">
                            ...
                            <div>
                                ...
                                <select id="tipe_hutang" style="padding:8px 12px;border-radius:6px;border:1px solid #e5e7eb;min-width:90px;">
                                    <option>Semua</option>
                                    <option>Pasien</option>
                                    <option>Distributor</option>
                                </select>
                            </div>
                            <input type="text" placeholder="Cari nama pasien, nama distributor atau nomor invoice" style="min-width:220px;">
                            <button type="submit" style="background:#64748b;color:#fff;padding:10px 18px;border-radius:6px;border:none;box-shadow:1px 2px 6px #e5e7eb">FILTER</button>
                        </form>
                    </div>
                    <div class="table-responsive">
                        <table>
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px;font-size:14px;color:#64748b;">
                        <div>Jumlah baris per halaman: <select style="border:none;background:transparent;color:#2563eb;font-weight:500;outline:none;">
                            <option>5</option><option>10</option><option>20</option></select></div>
                        <div>0-0 dari 0 data</div>
                        <div style="display:flex;gap:4px;align-items:center;">
                            <button style="background:none;border:none;color:#bdbdbd;font-size:18px;cursor:pointer;" disabled>&lt;|</button>
                            <button style="background:none;border:none;color:#bdbdbd;font-size:18px;cursor:pointer;" disabled>&lt;</button>
                            <button style="background:none;border:none;color:#bdbdbd;font-size:18px;cursor:pointer;" disabled>&gt;</button>
                            <button style="background:none;border:none;color:#bdbdbd;font-size:18px;cursor:pointer;" disabled>|&gt;</button>
                        </div>
        ...
        if(hdDropdownBtn && hdDropdownMenu){
            document.addEventListener('click', function(e) {
                if (hdDropdownBtn.contains(e.target)) {
                    hdDropdownMenu.style.display = hdDropdownMenu.style.display === 'block' ? 'none' : 'block';
